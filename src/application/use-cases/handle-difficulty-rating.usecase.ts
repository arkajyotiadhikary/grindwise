...
export class HandleDifficultyRatingUseCase {
  ...
  async execute(user: User, rating: 'EASY' | 'MEDIUM' | 'HARD'): Promise<void> {
    try {
      ...
      await this.messenger.sendText(user.phone_number, feedbackMessages[rating] ?? '');
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      console.error('[HandleDifficultyRating] execute failed', { error: message, phone: user.phone_number });
      throw err;
    }
  }
}
