...
export class HandleReviewRatingUseCase {
  ...
  async execute(
    ...
    try {
      ...
      this.repo.updateSpacedRepetition(
        user.id,
        firstReview.topic_id,
        qualityMap[rating] ?? 0,
      );

      const messages: Record<string, string> = {
        RECALL:
          "🟢 Perfect recall! This topic's next review has been pushed further out.",
        FUZZY: "🟡 Partial recall noted. We'll review this again soon.",
        BLANK:
          "🔴 No worries — this happens! It'll be reviewed again tomorrow.",
      };

      await this.messenger.sendText(user.phone_number, messages[rating] ?? '');
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      console.error('[HandleReviewRating] execute failed', {
        error: message,
