...
export class SqliteRepositoryAdapter implements IRepositoryPort {
  ...
  getTopicById(id: string): Topic | undefined {
    return this.db.prepare('SELECT * FROM topics WHERE id = ?').get(id) as
      | Topic
      | undefined;
  }
  ...
  getAllTopics(roadmapId = 'neetcode'): Topic[] {
    return this.db
      .prepare('SELECT * FROM topics WHERE roadmap_id = ? ORDER BY order_index')
  ...
  getProblemForTopic(topicId: string): Problem | undefined {
    return this.db
      .prepare('SELECT * FROM problems WHERE topic_id = ? LIMIT 1')
      .get(topicId) as Problem | undefined;
  }

  getProblemsForTopic(topicId: string): Problem[] {
    return this.db
      .prepare('SELECT * FROM problems WHERE topic_id = ?')
      .all(topicId) as Problem[];
  }
  ...
  upsertProblem(
    ...
    `,
      ...
      .run(
        ...
        problem.leetcode_slug ?? null,
        problem.title,
        problem.description ?? null,
        problem.difficulty ?? 'Medium',
        problem.solution_code ?? null,
  ...
  getOrCreateProgress(userId: string, topicId: string): UserProgress {
    const existing = this.db
      .prepare('SELECT * FROM user_progress WHERE user_id = ? AND topic_id = ?')
    ...
    return this.db
      .prepare('SELECT * FROM user_progress WHERE id = ?')
      .get(id) as UserProgress;
  }
  ...
  submitTestAnswer(
    ...
    const test = this.db
      ...
      .get(testId) as WeeklyTest | undefined;
    if (!test) return 0;
